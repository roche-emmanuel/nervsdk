...
auto sample2_sub_range(const Vector<SampleVec2d>& cline, F64 startT, F64 endT,
                       bool normalizeOutputT, F64 minPointDist)
    -> Vector<SampleVec2d> {
    ...
    F64 clineMinT = cline.front().t;
    F64 clineMaxT = cline.back().t;

    if (endT < clineMinT || startT > clineMaxT) {
        return {};
    }

    F64 clampedStartT = std::clamp(startT, clineMinT, clineMaxT);
    F64 clampedEndT = std::clamp(endT, clineMinT, clineMaxT);
    ...
    for (I32 i = 1; i < num; ++i) {
        if (startIdx == -1 && cline[i - 1].t <= clampedStartT &&
            ...
            useStartPt = (cline[i].v - startPt).length() > minPointDist;
            startIdx = i;
        }
        if (endIdx == -1 && cline[i - 1].t <= clampedEndT &&
            ...
            useEndPt = (cline[i - 1].v - endPt).length() > minPointDist;
            endIdx = i - 1;
            break;
        }
    }
    ...
    if (n == 0) {
        ...
        if (ncopy > 0) {
            SampleVec2d* ptr = out.data() + (useStartPt ? 1 : 0);
            memcpy(ptr, &cline[startIdx], ncopy * sizeof(SampleVec2d));
        }
    }

    if (normalizeOutputT) {
        F64 trange = clampedEndT - clampedStartT;
        for (auto& s : out) {
