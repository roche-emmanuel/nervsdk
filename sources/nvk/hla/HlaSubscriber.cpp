...
// All OpenRTI includes are local to this .cpp — none leak into the public header.
// Include root is .../OpenRTI/include/rti1516e/ so <RTI/xxx.h> resolves correctly.
...
auto HlaSubscriber::connect(const HlaFederateConfig& cfg) -> bool
{
    ...
    try {
        ...
        logERROR("HlaSubscriber: failed to create RTI ambassador: {}",
                 std::string(e.what().begin(), e.what().end()));
        _impl.reset();
        return false;
    }
    ...
    try {
        ...
        logERROR("HlaSubscriber: createFederationExecution failed: {}",
                 std::string(e.what().begin(), e.what().end()));
        try { _impl->rti->disconnect(); } catch (...) {}
        _impl.reset();
        return false;
    }
    ...
    if (!lookup_handles()) {
        ...
        try { _impl->rti->resignFederationExecution(rti1516e::NO_ACTION); } catch (...) {}
        try { _impl->rti->disconnect(); } catch (...) {}
        _impl.reset();
        return false;
    }
    ...
    while (!_ambassador->timeConstrainedReady.load(std::memory_order_acquire)) {
        try {
            ...
            logERROR("HlaSubscriber: evokeMultipleCallbacks during TC handshake: {}",
                     std::string(e.what().begin(), e.what().end()));
            break;
        }
        if (std::chrono::steady_clock::now() > deadline) {
            logERROR("HlaSubscriber: timed out waiting for time-constrained grant.");
            try { _impl->rti->resignFederationExecution(rti1516e::NO_ACTION); } catch (...) {}
            try { _impl->rti->disconnect(); } catch (...) {}
            _impl.reset();
            return false;
...
void HlaSubscriber::disconnect()
{
    ...
    if (_impl->rti) {
        ...
        try {
            _impl->rti->destroyFederationExecution(to_wstr(_config.federationName));
...
auto HlaSubscriber::is_connected() const noexcept -> bool
{
    return _connected;
}
...
void HlaSubscriber::tick(F64 maxSeconds)
{
    ...
    static thread_local std::vector<HlaSubscriberAmbassador::EntityEvent> events;
    events.clear();
...
auto HlaSubscriber::sim_time() const noexcept -> F64
{
    return _simTime;
}
...
{
    try {
        _impl->classBaseEntity   = _impl->rti->getObjectClassHandle(L"BaseEntity");
        _impl->attrWorldLocation = _impl->rti->getAttributeHandle(_impl->classBaseEntity, L"WorldLocation");
        _impl->attrOrientation   = _impl->rti->getAttributeHandle(_impl->classBaseEntity, L"Orientation");
        _impl->attrEntityType    = _impl->rti->getAttributeHandle(_impl->classBaseEntity, L"EntityType");
        _impl->attrEntityId      = _impl->rti->getAttributeHandle(_impl->classBaseEntity, L"EntityIdentifier");
    ...
    _ambassador->init(_impl->classBaseEntity,
                      _impl->attrWorldLocation,
                      _impl->attrOrientation,
                      _impl->attrEntityType,
                      _impl->attrEntityId);
...
void HlaSubscriber::subscribe_entity_classes()
{
    ...
    try {
        _impl->rti->subscribeObjectClassAttributes(_impl->classBaseEntity, attrs, true);
