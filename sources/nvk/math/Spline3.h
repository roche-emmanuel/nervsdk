#ifndef _NV_SPLINE3_H_
#define _NV_SPLINE3_H_
...
template <typename T> struct ControlPoint3 {
    using value_t = T;

    Vec3<T> position;
    Vec3<T> tangent_in;  // Arriving tangent (from previous point)
    Vec3<T> tangent_out; // Departing tangent (to next point)

    constexpr ControlPoint3()
        : position(0, 0, 0), tangent_in(0, 0, 0), tangent_out(0, 0, 0) {}

    constexpr ControlPoint3(const Vec3<T>& pos)
        : position(pos), tangent_in(0, 0, 0), tangent_out(0, 0, 0) {}

    constexpr ControlPoint3(const Vec3<T>& pos, const Vec3<T>& tan_in,
                            const Vec3<T>& tan_out)
        : position(pos), tangent_in(tan_in), tangent_out(tan_out) {}

    // Auto-tangent: smooth tangent based on neighboring points
    void autoTangent(const Vec3<T>& prev, const Vec3<T>& next,
                     value_t tension = 0.5) {
        Vec3<T> dir = (next - prev) * tension;
        tangent_in = -dir;
        tangent_out = dir;
    }
};

template <typename T> class Spline3 {
  public:
    using value_t = T;
    using point_t = Vec3<T>;
    using control_point_t = ControlPoint3<T>;
  ...
  private:
    ...
    static value_t ddh00(value_t t) { return 12 * t - 6; }
    static value_t ddh10(value_t t) { return 6 * t - 4; }
    static value_t ddh01(value_t t) { return -12 * t + 6; }
    static value_t ddh11(value_t t) { return 6 * t - 2; }
  ...
  public:
    ...
    explicit Spline3(const Vector<point_t>& points, bool closed = false)
        : _closed(closed) {
    ...
    explicit Spline3(const Vector<control_point_t>& controlPoints,
                     bool closed = false)
    ...
    // Evaluate tangent (first derivative) on segment
    ...
    // Evaluate second derivative on segment (for curvature)
    [[nodiscard]] point_t getSegmentAcceleration(size_t segIdx,
                                                 value_t t) const {
        if (segIdx >= numSegments())
            return point_t(0, 0, 0);
        ...
        return p0.position * ddh00(t) + p0.tangent_out * ddh10(t) +
               p1.position * ddh01(t) + p1.tangent_in * ddh11(t);
    }

    // Evaluate point using global t [0,1] across entire spline
    [[nodiscard]] point_t evaluate(value_t t) const {
        if (_control_points.size() < 2) {
            return _control_points.empty() ? point_t(0, 0, 0)
    ...
    [[nodiscard]] point_t evaluateDirection(value_t t) const {
        ...
        return len > 0 ? tangent / len : point_t(1, 0, 0);
    }

    // Evaluate curvature at t
    [[nodiscard]] value_t evaluateCurvature(value_t t) const {
        if (_control_points.size() < 2)
            return 0;
        ...
        point_t velocity = getSegmentTangent(segIdx, localT);
        point_t accel = getSegmentAcceleration(segIdx, localT);

        point_t cross = velocity.cross(accel);
        value_t crossLen = cross.length();
        value_t velLen = velocity.length();

        if (velLen < 1e-10)
            return 0;

        return crossLen / (velLen * velLen * velLen);
    }

    // Get Frenet-Serret frame at t (tangent, normal, binormal)
    struct FrenetFrame {
        point_t tangent;
        point_t normal;
        point_t binormal;
    ...
    [[nodiscard]] FrenetFrame evaluateFrenetFrame(value_t t) const {
        FrenetFrame frame;

        if (_control_points.size() < 2) {
            frame.tangent = point_t(1, 0, 0);
            frame.normal = point_t(0, 1, 0);
            frame.binormal = point_t(0, 0, 1);
            return frame;
        }
        ...
        value_t velLen = velocity.length();
        if (velLen > 1e-10) {
            frame.tangent = velocity / velLen;
        } else {
            frame.tangent = point_t(1, 0, 0);
        }

        // Binormal
        frame.binormal = velocity.cross(accel);
        value_t binormLen = frame.binormal.length();
        if (binormLen > 1e-10) {
            frame.binormal = frame.binormal / binormLen;
        } else {
            // Default binormal if velocity and accel are parallel
            frame.binormal = point_t(0, 0, 1);
        }
    ...
    [[nodiscard]] Spline3 subSpline(value_t t0, value_t t1) const {
        if (t0 > t1)
            std::swap(t0, t1);
    ...
    // Project spline onto a plane (defined by normal)
    [[nodiscard]] Spline3 projectOntoPlane(const point_t& planeNormal,
                                           const point_t& planePoint) const {
        Spline3 result = *this;
        point_t normal = planeNormal.normalized();

        for (auto& cp : result._control_points) {
            point_t vec = cp.position - planePoint;
            value_t dist = vec.dot(normal);
            cp.position = cp.position - normal * dist;

            // Project tangents too
            value_t dist_in = cp.tangent_in.dot(normal);
            cp.tangent_in = cp.tangent_in - normal * dist_in;

            value_t dist_out = cp.tangent_out.dot(normal);
            cp.tangent_out = cp.tangent_out - normal * dist_out;
        }

        result.invalidateLengths();
        return result;
    }
...
using Spline3f = Spline3<F32>;
using Spline3d = Spline3<F64>;
